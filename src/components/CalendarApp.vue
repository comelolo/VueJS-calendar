    ...
    <v-spacer></v-spacer>
        ...
        >
         <template v-slot:item="items">
            ...
            <v-list-tile-content>
              <v-list-tile-title v-html="items.name"></v-list-tile-title>
              <v-list-tile-sub-title v-html="items.post"></v-list-tile-sub-title>
            </v-list-tile-content>
...
export default {
  ...
  data: vm => ({
    ...
    search: null,
    peoples: [
        { 'name': 'Côme LOLLIVIER', 'post': 'CEO' },
        { 'name': 'Pierre-Joseph Delaphosse', 'post': 'Commercial' },
        { 'name': 'Bastien Pederencino', 'post': 'Chief innovation' },
        { 'name': 'Leo Fritz', 'post': 'Engineer' },
        { 'name': 'Mathilde Millet', 'post': 'Engineer' },
        { 'name': 'Jane Smith ', 'post': 'Engineer' },
        { 'name': 'John Smith', 'post': 'Engineer' },
        { 'name': 'Sandra Williams', 'post': 'Engineer' }
      ]
  }),
  ...
  computed:
  {
    ...
    items () {
      return this.result.map(people => {
        const name = people.name.length > this.descriptionLimit
          ? people.name.slice(0, this.descriptionLimit) + '...'
          : people.name

        const post = people.post
        
        return Object.assign({}, people, { name, post })
      })
    },
